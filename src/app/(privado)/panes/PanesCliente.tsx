...
export function PanesCliente({ panesIniciales }: Props) {
  ...
  function abrirEditar(pan: PanConCosto) {
    setEditando(pan)
    setModalAbierto(true)
  }
  ...
  return (
    <>
      <div className="flex justify-between items-center mb-4">
        <p className="text-sm text-gray-500">{panesIniciales.length} panes registrados</p>
        <Button onClick={abrirCrear}>+ Nuevo Pan</Button>
      </div>

      {panesIniciales.length === 0 ? (
        <div className="text-center py-16 text-gray-400">
          <p>No hay panes registrados.</p>
          <Button onClick={abrirCrear} className="mt-4">Registrar primer pan</Button>
        </div>
      ) : (
        <TablaPanes panes={panesIniciales} onEditar={abrirEditar} />
      )}
